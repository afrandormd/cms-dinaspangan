...
import { saveGallery, deleteGallery, saveSliders, deleteSliders } from '@/services/site';
...
import { deleteKategoriLayanan, saveKategoriLayanan, updateKategoriLayanan } from '@/services/master-data';

const KategoriLayanan = ({data}:{data:any}) => {
    ...
    const save = async () => {
        ...
        const saveing = await saveKategoriLayanan({
            namaKategori: judul
        })
        if(saveing.status) {
            notifications.show({
    ...
    const confirmUpdate = async () =>{
        setLoading(true);
        const update = await updateKategoriLayanan({
            id: parseInt(id),
            namaKategori: judul
        });
        if(update.status) {
    ...
    const confirmDelete = async () => {
        const del = await deleteKategoriLayanan({
            id: parseInt(id)
        });
        if(del.status) {
            notifications.show({
